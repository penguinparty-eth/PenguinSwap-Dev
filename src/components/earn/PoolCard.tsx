...
export default function PoolCard({ stakingInfo }: { stakingInfo: StakingInfo }) {
  ...
  if (totalSupplyOfStakingToken && stakingTokenPair) {
    ...
    valueOfTotalStakedAmountInWETH = new TokenAmount(
      ...
      JSBI.divide(
        JSBI.multiply(
          JSBI.multiply(stakingInfo.totalStakedAmount.raw, stakingTokenPair.reserveOf(FISH).raw),
  ...
  const USDPrice = useUSDCPrice(FISH)
      ...
      <StatContainer>
        ...
        <RowBetween>
          ...
          <TYPE.white>{`${stakingInfo.totalRewardRate
            ?.multiply(`${60 * 60 * 24 * 7}`)
            ?.toFixed(0, { groupSeparator: ',' })} 🐟 / week`}</TYPE.white>
        </RowBetween>
      </StatContainer>
